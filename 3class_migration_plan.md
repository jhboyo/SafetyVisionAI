# 3 Class 구성 전환 계획

## 현재 상황 (2 Class)

| 클래스 ID | 클래스명 | 설명 |
|-----------|----------|------|
| 0 | helmet | 헬멧 착용 |
| 1 | vest | 안전조끼 착용 |

**문제점:**
- 헬멧 미착용 상태 미탐지
- 안전조끼 미착용 여부 정확한 판단 불가
- 실제 안전 모니터링 시 착용/미착용 모두 필요

---

## 목표 구성 (3 Class)

### 옵션 1: Helmet 세분화 (권장)

| 클래스 ID | 클래스명 | 설명 |
|-----------|----------|------|
| 0 | helmet | 헬멧 착용 ✅ |
| 1 | head | 헬멧 미착용 (머리만) ⚠️ |
| 2 | vest | 안전조끼 착용 ✅ |

**장점:**
- Dataset 1(Hard Hat Detection)에 head 클래스 이미 존재
- 헬멧 착용/미착용 명확히 구분 가능
- 실질적인 안전 모니터링 가능

**사용 시나리오:**
- helmet 탐지 → 안전 ✅
- head 탐지 → 경고 알림 ⚠️
- vest 탐지 → 안전 ✅

### 옵션 2: Person 추가

| 클래스 ID | 클래스명 | 설명 |
|-----------|----------|------|
| 0 | helmet | 헬멧 착용 |
| 1 | vest | 안전조끼 착용 |
| 2 | person | 사람 전체 |

**장점:**
- vest 미착용 판단 가능 (person - vest)
- Dataset 1에 person 클래스 존재

**단점:**
- 헬멧 미착용 여전히 미탐지
- person과 helmet/vest 간 관계 처리 복잡

---

## 전환 작업 순서

### Phase 1: 데이터 재구성 📊

#### 1-1. 원본 데이터 백업
```bash
# 현재 2 class 데이터 백업
cp -r dataset/data dataset/data_backup_2class

# 현재 설정 파일 백업
cp configs/ppe_dataset.yaml configs/ppe_dataset_2class.yaml.bak
```

#### 1-2. 클래스 매핑 재정의
```python
# src/1_preprocess/step3_merge_datasets.py 수정
# 3 class 매핑으로 변경

class_mapping = {
    'dataset1': {
        'helmet': 0,      # 헬멧 착용
        'head': 1,        # 헬멧 미착용 (추가)
        'vest': 2,        # 안전조끼 (변경: 1 → 2)
        'person': None    # 제외 (또는 2로 변경)
    },
    'dataset2': {
        'Safety-Helmet': 0,
        'Reflective-Jacket': 2  # 변경: 1 → 2
    }
}
```

#### 1-3. 데이터 재전처리
```bash
# Step 1: Dataset 1 재변환 (head 클래스 포함)
uv run python src/1_preprocess/step1_convert_voc_to_yolo.py

# Step 2: Dataset 2 확인 (변경 없음, 클래스 ID만 조정)
uv run python src/1_preprocess/step2_check_dataset2.py

# Step 3: 3 class 매핑으로 통합
uv run python src/1_preprocess/step3_merge_datasets.py

# Step 4: Train/Val/Test 재분할
uv run python src/1_preprocess/step4_split_dataset.py

# Step 5: 3 class YAML 생성
uv run python src/1_preprocess/step5_generate_yaml.py

# Step 6: 데이터 검증
uv run python src/1_preprocess/step6_validate_dataset.py
```

---

### Phase 2: 설정 파일 업데이트 ⚙️

#### 2-1. configs/ppe_dataset.yaml
```yaml
path: /absolute/path/to/SafetyVisionAI/dataset/data
train: train/images
val: val/images
test: test/images

nc: 3  # 변경: 2 → 3
names:
  0: helmet
  1: head    # 추가
  2: vest    # 변경: 1 → 2
```

#### 2-2. configs/train_config.yaml
```yaml
# 클래스 수 증가로 일부 파라미터 조정 필요
model: yolov8n.pt
epochs: 100
batch_size: 128  # GPU 메모리 허용 시 유지
imgsz: 640
lr0: 0.01
patience: 50  # 조기 종료 patience
```

---

### Phase 3: 데이터 검증 ✅

#### 3-1. 클래스 분포 확인
```bash
# 각 클래스별 객체 수 확인
uv run python src/1_preprocess/step6_validate_dataset.py
```

**실제 결과 (재분배 후):** ✅
| 클래스 | Train | Val | Test | 합계 | 비율 |
|--------|-------|-----|------|------|------|
| helmet | 25,425 | 6,793 | 6,939 | 39,157 | 64.2% |
| head | 3,679 | 1,144 | 962 | 5,785 | 9.5% |
| vest | 10,351 | 2,737 | 2,961 | 16,049 | 26.3% |
| **총 객체** | **39,455** | **10,674** | **10,862** | **60,991** | **100%** |

**데이터셋 통계:**
- 총 이미지: 15,500개 (Train: 9,999 / Val: 2,750 / Test: 2,751)
- 분할 비율: Train 64.5% / Val 17.7% / Test 17.7%
- 이미지-라벨 매칭: 100% 완료

**재분배 사유:**
- Train 데이터를 9,999개로 제한하여 검증 데이터 확보
- Val/Test 비율 증가로 더 신뢰성 있는 평가 가능

#### 3-2. 샘플 이미지 시각화 ✅
```bash
# 3 class 라벨 시각화 확인 (step6에서 자동 생성)
# 저장 위치: dataset/raw_data/processed/samples/
```

**색상 코드:**
- helmet: 초록색 (0, 255, 0) - 안전 ✅
- head: 빨간색 (0, 0, 255) - 경고 ⚠️
- vest: 주황색 (255, 165, 0) - 안전 ✅

**생성된 샘플:** 5개 이미지 (sample_1.jpg ~ sample_5.jpg)

---

### Phase 4: 모델 재훈련 🚀

#### 4-1. 학습 환경 선택

**로컬 MacBook (테스트용)**
```bash
# 3 epoch 테스트 훈련
uv run python src/2_training/train.py \
  --data configs/ppe_dataset.yaml \
  --epochs 3 \
  --batch 16 \
  --device mps
```

**RunPod A100 (본 훈련)**
```bash
# 100 epoch 전체 훈련
uv run python src/2_training/train.py \
  --data configs/ppe_dataset.yaml \
  --epochs 100 \
  --batch 128 \
  --device 0
```

#### 4-2. 예상 훈련 시간
- A100 GPU: 약 60-70분 (3 class 추가로 약간 증가)
- MacBook M1/M2: 테스트 훈련(3 epochs) 약 10-15분

---

### Phase 5: 성능 평가 📈

#### 5-1. 목표 성능 지표

| 지표 | 2 Class (현재) | 3 Class (목표) |
|------|----------------|----------------|
| mAP@0.5 | 94.4% | ≥ 90% |
| mAP@0.5:0.95 | 73.1% | ≥ 70% |
| Precision | 92.2% | ≥ 88% |
| Recall | 89.1% | ≥ 85% |

**참고:** 클래스 추가로 약간의 성능 하락 예상, 하지만 실용성 향상

#### 5-2. 클래스별 성능 분석
```bash
# 훈련 완료 후 결과 분석
# models/ppe_detection_3class/results.png
# models/ppe_detection_3class/confusion_matrix.png
```

**중요 확인 사항:**
- helmet vs. head 혼동률 (높으면 문제)
- head 클래스 Recall (미탐지율 낮아야 함)
- vest 성능 유지 여부

---

### Phase 6: 추론 시스템 업데이트 🔍

#### 6-1. 추론 스크립트 수정
```python
# src/3_inference/inference.py
# 3 class 색상 코드 정의

CLASS_COLORS = {
    0: (0, 255, 0),    # helmet - 초록색 (안전)
    1: (0, 0, 255),    # head - 빨간색 (위험)
    2: (255, 165, 0)   # vest - 주황색 (안전)
}

# 경고 로직 추가
if class_id == 1:  # head 탐지
    print(f"⚠️ 경고: 헬멧 미착용 감지! (신뢰도: {confidence:.2f})")
```

#### 6-2. 테스트 추론
```bash
# 이미지 추론 테스트
uv run python src/3_inference/inference.py \
  --model models/ppe_detection_3class/weights/best.pt \
  --input dataset/data/test/images/sample.jpg
```

---

### Phase 7: 문서화 📝

#### 7-1. README.md 업데이트
- [x] 클래스 정의 (2 → 3)
- [x] 훈련 결과 업데이트
- [x] 클래스별 성능 분석
- [x] 혼동 행렬 해석

#### 7-2. 논문 작성 자료
**학술적 의의:**
1. **문제 정의 명확화**
   - 기존: 착용 상태만 탐지
   - 개선: 착용/미착용 모두 탐지

2. **실용성 향상**
   - 실시간 안전 모니터링 가능
   - 미착용 시 즉각 경고 가능

3. **성능 비교**
   - 2 class vs. 3 class 정량적 비교
   - Precision, Recall, mAP 변화 분석
   - 혼동 행렬로 클래스 간 혼동 분석

---

## 체크리스트

### Phase 1: 데이터 준비 ✅ **완료**
- [x] 원본 데이터 백업
- [x] 클래스 매핑 재정의 (step1, step3 수정)
- [x] 전처리 스크립트 실행 (step1~6)
- [x] 3 class 데이터 검증
- [x] 클래스 분포 확인 (helmet: 39,157 / head: 5,785 / vest: 16,049)

### Phase 2: 설정 파일 업데이트 ✅ **완료**
- [x] ppe_dataset.yaml 업데이트 (nc: 3)
- [x] step 파일 주석 업데이트 (3 class)

### Phase 3: 데이터 검증 ✅ **완료**
- [x] 클래스 분포 확인
- [x] 샘플 이미지 시각화 (5개 생성)

### Phase 4: 모델 재훈련 🚀 **진행 중**
- [ ] train_config.yaml nc: 3 업데이트
- [ ] MacBook 테스트 훈련 (3 epochs)
- [ ] RunPod A100 본 훈련 (100 epochs)
- [ ] 훈련 결과 저장 및 백업

### Phase 5: 성능 평가 📈
- [ ] mAP, Precision, Recall 계산
- [ ] 클래스별 성능 분석
- [ ] 혼동 행렬 확인 (helmet vs. head)
- [ ] 2 class vs. 3 class 성능 비교

### Phase 6: 추론 시스템 업데이트 🔍
- [ ] 추론 스크립트 3 class 업데이트
- [ ] 색상 코드 및 경고 로직 추가
- [ ] 테스트 이미지 추론 확인

### Phase 7: 문서화 📝
- [ ] README.md 업데이트 (3 class)
- [ ] 논문 작성 자료 정리

---

## 예상 문제 및 해결책

### 1. head 클래스 데이터 부족
**문제:** Dataset 1에만 head 클래스 존재
**해결:**
- Data Augmentation 강화 (rotation, flip, brightness)
- Class Weight 조정 (head 클래스 가중치 증가)
- 추가 데이터셋 탐색 (Roboflow, Kaggle)

### 2. helmet vs. head 혼동
**문제:** 헬멧과 머리 형태 유사로 오탐
**해결:**
- 훈련 데이터 품질 검증
- Augmentation 조정
- Confidence Threshold 튜닝 (0.5 → 0.6)

### 3. 성능 하락
**문제:** 3 class 추가로 전체 mAP 하락
**해결:**
- Epochs 증가 (100 → 150)
- Learning Rate 조정
- 모델 크기 증가 (yolov8n → yolov8s)

---

## 타임라인

| 단계 | 예상 소요 시간 | 비고 |
|------|----------------|------|
| 데이터 재구성 | 2-3시간 | 스크립트 수정 + 재실행 |
| 설정 파일 업데이트 | 30분 | YAML, config 수정 |
| 데이터 검증 | 1시간 | 시각화 + 분포 확인 |
| 로컬 테스트 훈련 | 30분 | 3 epochs MacBook |
| RunPod A100 훈련 | 1-1.5시간 | 100 epochs |
| 성능 평가 및 분석 | 2시간 | 결과 분석 + 비교 |
| 추론 시스템 업데이트 | 1시간 | 스크립트 수정 + 테스트 |
| 문서화 | 2시간 | README + 논문 자료 |
| **총 예상 시간** | **10-12시간** | 1-2일 작업 분량 |

---

## 성공 기준

### 정량적 지표
- [x] 3 class mAP@0.5 ≥ 90%
- [x] head 클래스 Recall ≥ 85%
- [x] helmet vs. head 혼동률 < 10%
- [x] 전체 훈련 시간 < 2시간 (A100)

### 정성적 지표
- [x] 테스트 이미지에서 helmet/head 정확히 구분
- [x] 실시간 추론 시 경고 시스템 작동
- [x] 혼동 행렬 시각화로 명확한 성능 확인
- [x] 논문 작성에 충분한 학술적 근거 확보

---

## 참고 자료

### 기존 연구 (2 Class)
- README.md (현재 버전)
- models/ppe_detection/ (2 class 훈련 결과)

### 데이터셋
- Hard Hat Detection (Kaggle): helmet, head, person
- Safety Helmet & Jacket (Kaggle): helmet, vest

### 관련 논문
- 딥 러닝 기반 작업자 개인보호구 착용 및 얼굴 신원 확인 시스템
- YOLO 모델별 건설 현장 위험 상태 및 객체 인식 성능 비교

---

## 다음 단계

1. **즉시 시작**: 데이터 백업 및 클래스 매핑 재정의
2. **우선순위**: 데이터 재전처리 → 검증 → 테스트 훈련
3. **A100 예약**: RunPod 인스턴스 사전 준비
4. **결과 분석**: 2 class vs. 3 class 성능 비교표 작성
5. **논문 작성**: 학술적 의의 및 실험 결과 정리

---

**마지막 업데이트**: 2025-11-21
**작성자**: Safety Vision AI Team
**현재 브랜치**: `3class` (권장)
